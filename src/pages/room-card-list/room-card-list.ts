...
/**
 * Generated class for the RoomCardListPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-room-card-list',
  templateUrl: 'room-card-list.html',
})
export class RoomCardListPage {
  ...
  loadData(){
    this.httpSerProvider.get('/equipment/getCardlist',this.person).then((data:any)=>{
          if(data.code==='0000'){
  ...
  public allStatus(one:any){
    ...
    window.setTimeout(() =>this.adminUnlock(this.roomList[one].roomNo,this.roomList[one].cardNo,this.roomList[one].id),1000+one*4000);
  ...
  public adminUnlock(userCodeStr,cardCodeStr,id){
    ...
    playRecord(){ 
        var model = this.media.create(this.fileUrl.replace(/^file:\/\//, '')+ "RecordCard.wav");
        model.play();
    }
    stopRecord(){   
         ...
         if(count=='100'){
              ...
              if(this.delState == true){
                ...
                //第二次开锁无论成功或失败，都会执行该方法
                  ...
                  this.httpSerProvider.post('/equipment/updateCardState',person1).then((data:any)=>{
                        if(data.code==='0000'){
                          if(this.delState == true){
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.cardEquipmentId = this.person.equipmentId;  
            this.adminUnlock(opt.roomNo,opt.cardNo,this.cardCodeId);
